...
const OpenLead = () => {
  ...
  const renderStats = () => {
    ...
    const salesData = [
      { stats: counts.todayFollowup, title: 'Today Leads', color: 'primary', icon: <AccountBalanceWalletIcon sx={{ fontSize: '1.75rem' }} /> },
      ...
      { stats: counts.totalFollowup, title: 'Total Followup', color: 'info', icon: <CancelIcon sx={{ fontSize: '1.75rem' }} /> }
    ];
  ...
  const getPieData = () => {
    ...
    return [
      ...
      { name: 'Backlog Pending', value: counts.backlogFollowup, color: '#4caf50' },
      { name: 'Tranfer to Opportunity', value: counts.transfertooppo, color: '#ff9800' },
      { name: 'Total Follow Up', value: counts.totalFollowup, color: '#00acc1' }
    ];
  };
      ...
      case 'history':
        return (
          <Box display="flex" flexDirection="column" alignItems="center" justifyContent="center" minHeight="100vh">
            ...
            {/* Replace with actual history component */}
          </Box>
        );
      default:
        return <WelcomeScreen />;
